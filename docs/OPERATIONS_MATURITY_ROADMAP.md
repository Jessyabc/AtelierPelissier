...
Checklist:
- Clear handoff moment exists — the "Save" button (isDraft → false), but no required-field enforcement
- Required data for production start explicit — readiness check defined but not enforced
- Product descriptions standardized enough for machine interpretation — partial
- Missing measurements/materials/finishes flagged — partial (deviation engine catches some)
- System can block progression on missing required fields — NOT ENFORCED

**Score: 2 / 5**  
**Owner:** Sales + Planner  
**Actions:**
- Enforce readiness gate hard block (P1-002)
- Add missing-info flags prominently on project detail for salesperson view
...
- Invoice PDF/confirmation identification — manual upload only
- Parser extracts invoice #, supplier, items, qty, dates — NOT IMPLEMENTED (upload exists, parsing does not)
- Parsed docs linked to projects with confidence thresholds — NOT IMPLEMENTED
- Human confirmation for low-confidence matches — NOT IMPLEMENTED
...
|---------|-------|--------------|
| 1. Core operational foundation | 2 | — |
| 2. Project intake | 3 | — |
| 3. Sales-to-production handoff | 2 | — |
| 4. Email/document ingestion | 1 | — |
| 5. Project matching | 2 | — |
| 6. Production workflow + task distribution | **2** | ↓ was 3 |
| 7. QR station timing | 3 | — |
| 8. Standard time baselines | **0** | ↓ was 1 |
| 9. Inventory + purchasing | 4 | — |
| 10. Cutlist integration | 3 | — |
| 11. Roles and permissions | **2** | ↓ was 3 |
...
| **Overall average** | **2.1 / 5** | ↓ from 2.4 (audit revealed deeper gaps) |
