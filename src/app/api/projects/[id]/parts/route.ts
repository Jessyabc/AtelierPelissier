...
// GET panel parts — any authenticated user. Cutlist reads power the
// CutList tab (planner) and the observability engine's material roll-up.
...
/**
 * DELETE parts: all for project (no query), or only project-level
 * (?cutlistId=null), or only one cutlist (?cutlistId=<cuid>).
 * Admin/planner only — cutlist is production data.
 */
...
export const POST = withAuth<{ id: string }>(
  ...
  async ({ req, params }) => {
    ...
    if (data.cutlistId != null) {
      const cutlistRepo = (prisma as unknown as {
        cutlist: {
          findFirst: (args: {
            where: { id: string };
            include: { projectItem: { select: { projectId: true } } };
          }) => Promise<{ projectItem: { projectId: string } } | null>;
        };
      }).cutlist;
