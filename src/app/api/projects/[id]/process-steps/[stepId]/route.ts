 ...
 * Admin/planner only — orchestration of production.
 */
export const PATCH = withAuth<StepParams>(
  ["admin", "planner"],
  async ({ req, params }) => {
...
/**
 * DELETE: Remove a ProjectProcessStep. Admin/planner only.
 */
export const DELETE = withAuth<StepParams>(
  ["admin", "planner"],
  async ({ params }) => {
    const { id: projectId, stepId } = params;
