...
describe("kitchen pricing engine", () => {
  ...
  it("excludes installation from totals when installationTbd is true", () => {
    const payload = {
      roomDefaults: staticKitchenRoomDefaults(),
      cabinets: [
        {
    ...
    const totals = calculateTotalCost(payload);
    expect(totals.installationSubtotal).toBe(0);
    expect(totals.deliverySubtotal).toBe(500);
    expect(totals.totalCost).toBe(1062.2);
  });

  it("excludes delivery from totals when deliveryTbd is true", () => {
    const payload = {
      roomDefaults: staticKitchenRoomDefaults(),
      cabinets: [
        {
    ...
    const totals = calculateTotalCost(payload);
    expect(totals.installationSubtotal).toBe(75);
    expect(totals.deliverySubtotal).toBe(0);
    expect(totals.totalCost).toBe(637.2);
  });
});
